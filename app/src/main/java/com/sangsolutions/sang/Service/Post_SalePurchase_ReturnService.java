...
public class Post_SalePurchase_ReturnService extends JobService {
    ...
    private void GetDataSalePurchaseReturn() {
        @SuppressLint("StaticFieldLeak") AsyncTask<Void,Void,Void> asyncTask=new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... voids) {
                ...
                Cursor cursorHeader=helper.getDataFromS_P_return_HeaderPost();
                Log.d("responsePostReturn", cursorHeader.getCount()+"mmmm");
                if(cursorHeader.moveToFirst() && cursorHeader.getCount()>0){
                    for (int i=0;i<cursorHeader.getCount();i++){
                        ...
                        try {
                            ...
                            Cursor cursorBody=helper.getEditValuesBodyS_P_return(iTransId,iDocType);
                            ...
                            if(cursorBody.moveToFirst() && cursorBody.getCount()>0){
                                ...
                                for (int j=0;j<cursorBody.getCount();j++){
                                    ...
                                    jsonObject.put("iTransReturnId",cursorBody.getInt(cursorBody.getColumnIndex(Sales_purchase_Class.I_TRANS_RETURN_ID)));
    ...
    private void uploadToAPI(JSONObject jsonObjectMain, String docNo, int iTransId, int iDocType) {
        AndroidNetworking.post("http://"+ new Tools().getIP(this)+ URLs.PostTransReturn)
